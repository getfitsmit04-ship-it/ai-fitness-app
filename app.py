...
class PreviousLog(db.Model):
    id = db.Column(db.Integer, primary_key=True); exercise_name = db.Column(db.String(100), nullable=False); user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
...
EXERCISE_KNOWLEDGE_BASE = { 'cardio': [{'name': 'Treadmill', 'gif_url': '/static/assets/treadmill.gif', 'instructions': "<h4>Settings:</h4><ul><li><b>Speed:</b> 5-6 km/h (brisk walk) or 8-10 km/h (light jog).</li><li><b>Incline:</b> 1-2%.</li></ul>"}], 'stretches': [{'name': 'Quad Stretch', 'gif_url': '', 'instructions': "<p>Hold for 30 seconds per leg.</p>"}], 'main': { 'chest': [{'name': 'Vertical Chest Press', 'gif_url': '', 'instructions': "<h4>How-To:</h4><p>Sit with your back flat against the pad...</p>"}], 'back': [{'name': 'Lat Pull Down', 'gif_url': '', 'instructions': "<h4>How-To:</h4><p>Grab the bar with a wide, overhand grip...</p>"}], 'arms': [{'name': 'Bicep Curls Machine', 'gif_url': '', 'instructions': "<h4>How-To:</h4><p>Rest your arms on the pad...</p>"}], 'legs': [{'name': 'Leg Extension', 'gif_url': '', 'instructions': "<h4>How-To:</h4><p>Sit with your back against the pad...</p>"}], 'glutes': [{'name': 'Hip Thrust Machine', 'gif_url': '', 'instructions': "<h4>How-To:</h4><p>Position your back against the pad...</p>"}], 'abs': [{'name': 'Abdominal Machine', 'gif_url': '', 'instructions': "<h4>How-To:</h4><p>Crunch forward, bringing your chest towards your knees...</p>"}] } }
...
def generate_ai_workout_plan(user):
    ...
    for day in ["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"]:
        if day in split:
            ...
            if workout_type == 'Push': exercises_to_add.extend(random.sample(get_available_exercises('chest'), 1) + random.sample(get_available_exercises('arms'), 2))
            elif workout_type == 'Pull': exercises_to_add.extend(random.sample(get_available_exercises('back'), 2) + random.sample(get_available_exercises('arms'), 1))
            elif workout_type in ['Legs', 'Lower Body']: exercises_to_add.extend(random.sample(get_available_exercises('legs'), 2) + random.sample(get_available_exercises('glutes'), 1))
            else: exercises_to_add.extend(random.sample(get_available_exercises('chest'), 1) + random.sample(get_available_exercises('back'), 1) + random.sample(get_available_exercises('legs'), 1))
            for area in focus_areas:
                if area in EXERCISE_KNOWLEDGE_BASE['main']: exercises_to_add.append(random.choice(get_available_exercises(area)))
            for ex_obj in list({v['name']:v for v in exercises_to_add}.values()): # Remove duplicates
                ex_obj_copy = ex_obj.copy()
                suggestion = "<h4>Starting Weight:</h4><p>e.g. 15-25 kg</p>" # Simplified for first run
...
def reset_all_data(secret_key):
    if secret_key != 'your_secret_reset_key': return "Unauthorized", 403
    try:
