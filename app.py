...
# Use DATABASE_URL from environment if available (for Render), otherwise use SQLite
...
def generate_ai_workout_plan(user):
    ...
    if 'bold' in goals or 'strength' in goals:
        rep_range, rep_target = "4 sets of 6-8 reps", 6
    else:
        rep_range, rep_target = "3 sets of 10-12 reps", 10
    ...
    for day in day_names:
        if day in split:
            ...
            # Filter out exercises logged during signup
            def get_available_exercises(muscle_group):
                return [ex for ex in EXERCISE_KNOWLEDGE_BASE['main'][muscle_group] if ex['name'] not in previous_exercises] or EXERCISE_KNOWLEDGE_BASE['main'][muscle_group]

            if workout_type == 'Push':
                exercises_to_add.extend([
                    random.choice(get_available_exercises('chest')),
                    random.choice(get_available_exercises('shoulders')),
                    random.choice(get_available_exercises('arms')) # Simplified to 'arms'
                ])
            elif workout_type == 'Pull':
                exercises_to_add.extend([
                    random.choice(get_available_exercises('back')),
                    random.choice(get_available_exercises('back')),
                    random.choice(get_available_exercises('arms'))
                ])
            elif workout_type in ['Legs', 'Lower Body']:
                exercises_to_add.extend([
                    random.choice(get_available_exercises('legs')),
                    random.choice(get_available_exercises('legs')),
                    random.choice(get_available_exercises('glutes'))
                ])
            ...
            for stretch_obj in stretches:
                 workout['structure'].append({"type": "Cool-down", "details": stretch_obj, "duration": "30 seconds per side"})
...
def reset_all_data(secret_key):
    if secret_key != 'your_secret_reset_key':
